...
def test_sends_to_admin_number_with_unit_and_log_tail(tmp_path):
    ...
    send_alert("jobshub-backup.service", sender, "+91 99020 00000", log_path=log,
...
def test_refuses_without_a_valid_admin_number(tmp_path, phone):
    sender = FakeSender()
    with pytest.raises(RuntimeError, match="ADMIN_ALERT_WHATSAPP"):
        send_alert("x.service", sender, phone, log_path=tmp_path / "none.log")
    assert sender.sent == []
